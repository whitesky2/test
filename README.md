...
do {
            ...
            if let j_data = ori_data, let temp = j_data["data"] as? [String:AnyObject] {
                ...
                for i in 0..<arr_data.count {
                    /*if(i==arr_data.count) {
                     let label_width = ((arr_data[i-1].box?.width!)! != 500) ? (arr_data[i-1].location?.getWidth())!/2 : (arr_data[i-1].location?.getWidth())!/3
                     let label=UILabel(frame: CGRect(x: (arr_data[i-1].location?.left!)!+4, y: 4, width: label_width, height: (arr_data[i-1].location?.top!)!-4))
                     let txt=(arr_data[i-1].box?.label)!
                     
                     label.text=String(txt[txt.startIndex])
                     label.textAlignment = .center
                     label.backgroundColor = self.clickedColor
                    ...
                    if(columnIndex != arr_data[i].box?.col) // 열이 바뀌었을 때 sumOfWidth의 값을 늘려서 그릴 영역을 넓힘. 그리고 라벨링을 붙임.
                    {
                        /*if(columnIndex != -1)
                         {
                         let label=UILabel(frame: CGRect(x: (arr_data[i-1].location?.left!)!+4, y: 4, width: ((arr_data[i-1].location?.getWidth())!/3)-8, height: (arr_data[i-1].location?.top!)!-4))
                         let txt=(arr_data[i-1].box?.label)!
